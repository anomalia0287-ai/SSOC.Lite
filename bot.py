...
■ RED (즉시 조치) — 다음 중 하나 이상에 해당할 때만 RED:
  · 명시적 마감일이 24시간 이내이거나 이미 경과한 필수 조치
  · 법적 의무 위반 또는 규제 위반 가능성이 있는 긴급 사안
  · 현재 진행 중이거나 임박한 보안 사고 (랜섬웨어, 피싱 공격, 데이터 유출 등)
  · 즉각 서명 또는 승인이 필요한 문서 (미조치 시 법적·재무적 손실 발생)
  · 시스템 전면 장애 등 업무 중단을 초래하는 긴급 상황

■ YELLOW (당일 확인) — 중요하지만 즉각 조치가 불필요:
  · 마감일이 24시간 초과~7일 이내인 업무 안내
  · 정기 보안 교육·비밀번호 변경·SW 업데이트 권고 등 예방적 보안 안내
  · 시스템 정기 점검·일정 변경·인사 발령
  · VPN/보안솔루션 업데이트 등 기한에 여유가 있는 보안 조치 안내
  · 개인정보 처리방침 개정 고지 등 정보성 법무 공지

■ GREEN (참고) — 즉시 확인 불필요:
  · 일반 사내 정보, 복지·행사·동호회 안내, 문화 소식
  · 마감일이 7일 초과이거나 없는 선택적 참여 안내
  · 보안 뉴스레터, 보안 팁, 외부 동향 공유 등 참고용 보안 정보

⚠️ 분류 원칙:
  · "보안"이라는 키워드만으로 RED를 부여하지 마세요. 핵심 기준은 "시간적 긴급성"과 "미조치 시 실질적 피해 여부"입니다.
  · 정기적·반복적 보안 공지는 대부분 YELLOW 또는 GREEN에 해당합니다.
...
STAGE2_SYSTEM = """공지 분류 검증 전문가입니다.

⚠️ 중요: 아래에 1차 분석 참고 정보가 제공될 수 있으나, 반드시 원문만으로 독립적으로 판단하세요.
1차 분석과 다른 결론이 나와도 자신의 판단을 우선하세요.
...
⚠️ 분류 원칙:
  · "보안"이라는 키워드만으로 RED를 부여하지 마세요. 핵심 기준은 "시간적 긴급성"과 "미조치 시 실질적 피해 여부"입니다.
  · 정기적·반복적 보안 공지는 대부분 YELLOW 또는 GREEN에 해당합니다.
  · 1차 분석이 RED라 하더라도, 위 기준에 부합하지 않으면 YELLOW 또는 GREEN으로 재판정하세요.
...
def classify(text: str, channel: str = "") -> dict:
    ...
    prompt2  = (
        f"{notice}\n\n---\n"
        f"1차 분석 참고 (신뢰도 {r1['confidence']:.0%}): {r1['reason']}\n"
        f"※ 위 참고 정보와 무관하게, 원문 기준으로 독립 판단하세요."
    )
